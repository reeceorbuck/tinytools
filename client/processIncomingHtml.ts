    ...
    switch (mode) {
      case "attributes": {
        ...
        Array.from(partial.attributes).forEach((attr) => {
          if (
            attr.name !== "id" && attr.name !== "name" && attr.name !== "mode"
          ) {
            console.log(`Setting attribute ${attr.name} to ${attr.value}`);
            if (attr.name === "value") {
              (existing as HTMLInputElement).value = attr.value;
            } else {
      ...
      case "merge-content": {
        Array.from(partial.children).forEach((insertNode) => {
          const searchId = insertNode.getAttribute("match-id") ||
            insertNode.id;
          ...
          if (!existingChild) {
            ...
            existingChild = groupName
              ? Array.from(existing.children).find((eChild) => {
